...
class TestJobFinalizedEvent:
    ...
    ************************************************************
    ...
    def test_raises_validation_error_if_job_id_is_none(self):
        with pytest.raises(ValidationError):
            JobFinalizedEventData(job_id=None, job_name="Test", job_status="FINALIZED")

    def test_does_not_raise_if_job_id_is_empty(self):
        # Pydantic allows empty strings by default
        data = JobFinalizedEventData(job_id="", job_name="Test", job_status="FINALIZED")
        assert data.job_id == ""

    def test_raises_validation_error_if_job_name_is_none(self):
        with pytest.raises(ValidationError):
            JobFinalizedEventData(job_id="JOB-123", job_name=None, job_status="FINALIZED")

    def test_does_not_raise_if_job_name_is_empty(self):
        # Pydantic allows empty strings by default
        data = JobFinalizedEventData(job_id="JOB-123", job_name="", job_status="FINALIZED")
        assert data.job_name == ""

    def test_raises_validation_error_if_job_status_is_none(self):
        with pytest.raises(ValidationError):
            JobFinalizedEventData(job_id="JOB-123", job_name="Test", job_status=None)

    def test_does_not_raise_if_job_status_is_empty(self):
        # Pydantic allows empty strings by default
        data = JobFinalizedEventData(job_id="JOB-123", job_name="Test", job_status="")
        assert data.job_status == ""

    def test_raises_validation_error_if_missing_required_fields(self):
        with pytest.raises(ValidationError):
            JobFinalizedEventData(job_id="JOB-123")
    ...
    ************************************************************
    ...
    def test_from_data_creates_event_with_valid_data(self):
        event = JobFinalizedEvent.from_data(
            job_id="JOB-123", job_name="Test Job", job_status="FINALIZED"
        )
        assert isinstance(event, JobFinalizedEvent)
        assert event.idempotencyKey == "JOB_ID#JOB-123"
        assert event.createdAt is not None
        assert isinstance(event.eventData, JobFinalizedEventData)
        assert event.eventData.job_id == "JOB-123"
        assert event.eventData.job_name == "Test Job"
        assert event.eventData.job_status == "FINALIZED"

    def test_from_data_generates_correct_idempotency_key(self):
        event = JobFinalizedEvent.from_data(
            job_id="JOB-456", job_name="Test", job_status="FINALIZED"
        )
    ...
    def test_from_data_raises_validation_error_if_job_id_is_none(self):
        with pytest.raises(ValidationError):
            JobFinalizedEvent.from_data(job_id=None, job_name="Test", job_status="FINALIZED")

    def test_from_data_raises_validation_error_if_job_name_is_none(self):
        with pytest.raises(ValidationError):
            JobFinalizedEvent.from_data(job_id="JOB-123", job_name=None, job_status="FINALIZED")

    def test_from_data_raises_validation_error_if_job_status_is_none(self):
        with pytest.raises(ValidationError):
            JobFinalizedEvent.from_data(job_id="JOB-123", job_name="Test", job_status=None)
    ...
    ************************************************************
    ...
    def test_event_has_correct_event_name(self):
        ...
        assert event.eventName == "JOB_FINALIZED_EVENT"
    ...
    ************************************************************
    ...
    def test_event_can_be_serialized_to_dict(self):
        ...
        assert event_dict["eventName"] == "JOB_FINALIZED_EVENT"
        ...
        assert event_dict["eventData"]["job_status"] == "FINALIZED"

    def test_event_can_be_reconstituted_from_dict(self):
        original_event = JobFinalizedEvent.from_data(
            job_id="JOB-123", job_name="Test Job", job_status="FINALIZED"
        )
        event_dict = original_event.model_dump()
        reconstituted_event = JobFinalizedEvent.model_validate(event_dict)
