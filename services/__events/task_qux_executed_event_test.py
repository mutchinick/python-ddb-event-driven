...
from services.__events.task_qux_executed_event import TaskQuxExecutedEvent, TaskQuxExecutedEventData


class TestTaskQuxExecutedEvent:
    """
    ************************************************************
    ...
    ************************************************************
    ...
    def test_raises_validation_error_if_job_id_is_none(self):
        with pytest.raises(ValidationError):
            TaskQuxExecutedEventData(job_id=None, job_name="Test", job_status="EXECUTED")

    def test_does_not_raise_if_job_id_is_empty(self):
        # Pydantic allows empty strings by default
        data = TaskQuxExecutedEventData(job_id="", job_name="Test", job_status="EXECUTED")
        assert data.job_id == ""

    def test_raises_validation_error_if_job_name_is_none(self):
        with pytest.raises(ValidationError):
            TaskQuxExecutedEventData(job_id="JOB-123", job_name=None, job_status="EXECUTED")

    def test_does_not_raise_if_job_name_is_empty(self):
        # Pydantic allows empty strings by default
        data = TaskQuxExecutedEventData(job_id="JOB-123", job_name="", job_status="EXECUTED")
        assert data.job_name == ""

    def test_raises_validation_error_if_job_status_is_none(self):
        with pytest.raises(ValidationError):
            TaskQuxExecutedEventData(job_id="JOB-123", job_name="Test", job_status=None)

    def test_does_not_raise_if_job_status_is_empty(self):
        # Pydantic allows empty strings by default
        data = TaskQuxExecutedEventData(job_id="JOB-123", job_name="Test", job_status="")
        assert data.job_status == ""

    def test_raises_validation_error_if_missing_required_fields(self):
        with pytest.raises(ValidationError):
            TaskQuxExecutedEventData(job_id="JOB-123")
    ...
    ************************************************************
    ...
    def test_from_data_creates_event_with_valid_data(self):
        event = TaskQuxExecutedEvent.from_data(
            job_id="JOB-123", job_name="Test Job", job_status="EXECUTED"
        )
        assert isinstance(event, TaskQuxExecutedEvent)
        assert event.idempotencyKey == "JOB_ID#JOB-123"
        assert event.createdAt is not None
        assert isinstance(event.eventData, TaskQuxExecutedEventData)
    ...
    def test_from_data_raises_validation_error_if_job_id_is_none(self):
        with pytest.raises(ValidationError):
            TaskQuxExecutedEvent.from_data(job_id=None, job_name="Test", job_status="EXECUTED")

    def test_from_data_raises_validation_error_if_job_name_is_none(self):
        with pytest.raises(ValidationError):
            TaskQuxExecutedEvent.from_data(job_id="JOB-123", job_name=None, job_status="EXECUTED")

    def test_from_data_raises_validation_error_if_job_status_is_none(self):
        with pytest.raises(ValidationError):
            TaskQuxExecutedEvent.from_data(job_id="JOB-123", job_name="Test", job_status=None)
    ...
    ************************************************************
    ...
    def test_event_has_correct_event_name(self):
        ...
        assert event.eventName == "TASK_QUX_EXECUTED_EVENT"
    ...
    ************************************************************
    ...
    def test_event_can_be_serialized_to_dict(self):
        ...
        assert event_dict["eventName"] == "TASK_QUX_EXECUTED_EVENT"
    ...
    def test_event_can_be_reconstituted_from_dict(self):
        original_event = TaskQuxExecutedEvent.from_data(
            job_id="JOB-123", job_name="Test Job", job_status="EXECUTED"
        )
        event_dict = original_event.model_dump()
        reconstituted_event = TaskQuxExecutedEvent.model_validate(event_dict)
