...
def handler(sqs_event: SQSEvent, _context: Context) -> None:
    """
    SQS-triggered Lambda for JobStartedEvent events.
    """
    for sqs_record in sqs_event["Records"]:
        try:
            incoming_event = EventBase.from_eventbridge_sqs_record(sqs_record, JobStartedEvent)
            print(f"INFO: JobStartedEvent received with job ID: {incoming_event.idempotencyKey}")
        ...
        incoming_event_data: JobStartedEventData = incoming_event.eventData

        event = StepProcessedEvent(
